...
it("次問の再生成中は次へ進むボタンを無効にする", async () => {
  history.replaceState({}, "", "/?code=test-simple-addition");
  sessionStorage.setItem("practiceHellToken", "test-token");
  sessionStorage.setItem("practiceHellJoinCode", "test-simple-addition");
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, options?: RequestInit) => {
    ...
    if (path === "/api/session/question/regenerate" && options?.method === "POST") {
  ...
  fireEvent.change(await screen.findByLabelText("解答"), { target: { value: "4" } });
  fireEvent.click(screen.getByRole("button", { name: "解答する" }));
  fireEvent.click(await screen.findByRole("button", { name: "次の問題を再生成" }));

  expect(screen.getByRole("button", { name: "次の問題を生成中" })).toBeDisabled();
  expect(screen.getByRole("button", { name: "再生成中" })).toBeDisabled();

  finishRegeneration(Response.json({ ready: true }));
  await waitFor(() => {
